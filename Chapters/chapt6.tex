...
\label{sec:extreme}
	...
	\label{ssec:Facilities}
		...
		\label{sssec:GIF}
			...
			As described on Figure~\ref{fig:CsSource}, the $^{137}$Cs source with a half-life of 30 years, the activity of which has been measured on the 5$^{th}$ March 1997 to be \SI{740}{\GBq} (implying e.g. \SI{695}{\GBq} in December 1999), emits a \SI{662}{\keV} photon in 85\% of the decays. The principal collimator hole provides a pyramidal aperture of \SI{74}{\degree}$\times$ \SI{74}{\degree} solid angle. This provides a photon flux in a pyramidal volume of \SI{5}{\meter} maximum length along the beam axis.
...
\label{sec:GIFtests}
	...
	\subsection{Geometrical acceptance of the setup layout}
	\label{ssec:GeoAcc}
			...
			\begin{table}[!h]
				\begin{center}
					\begin{tabular}{|*{5}{c|}}
						...
						\hline
					\end{tabular}
					\caption{\label{tab:Sim1997} Total photon flux ($E\gamma \leq$ \SI{662}{\keV}) with statistical error predicted considering a $^{137}$Cs activity of \SI{740}{\GBq} at different values of the distance $D$ from the source along the x-axis of irradiation field~\cite{GIFacility}.}
				\end{center}
			\end{table}
