...
\label{sec:GIFtests}
	...
	\label{ssec:gFlux}
		...
		\label{sssec:Simulations}
			...
			The simulation doesn't directly provides us with an estimated flux at the level of our RPC. It will be needed to track back the value of the flux and extract it from the available data contained in the original paper. First of all, looking at Figure~\ref{fig:Sim1997} that contains the data from Table~\ref{tab:Sim1997}, it is possible to assume that the flux decreases as $1/D^2$ for the range of value that is avaible to us. Let's then consider that Formula~\ref{eq:Flux} will be used to described the behaviour of the flux where $D_0$ and $F^{ABS}_0$ are respectively the distance from a reference point to the source and flux at the level of this reference point and $c$ is a fitting factor.
			...
			Using Equation~\ref{eq:Factor} to fill Table~\ref{tab:CorrFactor}
			...
			\begin{table}[!h]
				\begin{center}
					\begin{tabular}{|*{4}{c|}}
						...
						\hline
					\end{tabular}
					\caption{\label{tab:CorrFactor} Correction factor c is computed thanks to formulae~\ref{eq:Factor} taking as reference $D_0 = 50 cm$ and the associated flux $F_0^{ABS}$ for each absorption factor available in table~\ref{tab:Sim1997}.}
				\end{center}
			\end{table}
			
			that can be approximated by the following linear function :\\
			
			\begin{equation}
				c\left(\frac{D}{D_0}\right) = a\frac{D}{D_0} + b
...
\label{sec:results6}
	...
	%****************************************** RESULTS AT GIF++ ********************************************************************
	\subsection{Consolidation studies results}
