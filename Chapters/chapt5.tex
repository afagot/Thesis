...
\label{chapt5:sec:GIFpptests}
	...
	At GIF++, the CMS RPC R\&D experiment collects different types of data coming from the detectors monitored parameters, such as voltage and currents, the gas, source, and environemental parameters, and, of course, the TDC data in which are collected the actual muon and gamma physics. These different data source compose 3 different data flows.
	
	A first, called \textit{DIP flow}, DIP being the name of the database in which the data is retrieved from, concerns all the data coming from the gas composition, temperature and humidity, the environmental temperature and pressure, the source settings and the radiation monitoring sensors. The experimental area is in charge of measuring, storing and distributing the data of interest for all of the users of the facility (source settings, radiation monitoring, gas composition at the exit of the gas mixer and general environmental information). Retrieving this data is done by accessing to the database of the experimental hall in which GIF++ is located. More specific data such as gas flow, temperature and humidity at the level of the detectors (upstream and downstream of the detectors) as well as environmental parameters are at the charge of the users. For this reason, several pressure, temperature and humidity sensors were installed on the gas distribution system of the RPC trolleys. The corresponding flow of data is saved into the local CMS RPC database and displayed on the front page of the WebDCS together with alerts in the case the values measured are out of optimal working range.
	
	The data flow related to the monitoring of detector high voltages and currents, referred to as \textit{CAEN flow} as a reference to the manufacturer of power supplies, is retrieved thanks to computer to main frame communications. Indeed, during the operations (continuous irradiation or measurement with beam), these values can be accessed directly through the bus of the main frame hosting the high voltage supplies. The way this data articulates with the physics data flow, also called \textit{DAQ flow}, will be described further after giving more details about this third data flow. As previously described, two types of measurement are performed on the chambers using the TDCs. On one hand, during the beam period, it is interesting to measure the efficiency of the RPCs with increasing voltage and, thus, data containing muon information will be recorded into ROOT files during what is called a HV scan. The data is collected during several runs, each corresponding to a different HV setting. In this case, the trigger is provided by the 3 scintillators placed along the beam line (upstream of the bunker, in the bunker near \texttt{T1} and dowstream of the bunker) and extra information for the reconstruction of the muon tracks is provided by the two tracking chambers that are constantly operated a full efficiency. Every time a trigger is sent to the TDCs, i.e. every time a muon is detected, knowing the time delay in between the trigger and the RPC signals, signals located in the right time window are extracted from the buffers and saved for later analysis. Signals are taken in a time window of \SI{400}{ns} centered on the muon peak.
	
	On the other hand, during the irradiation period but also during muon time, the goal is to measure the gamma background to have an idea of the strength of the irradiation recorded by the chamber. This way, during the irradiation, the data is used to control any change of sensibility of the detectors, the appearance of any hot channels that could be correlated to the appearance of hot spots, regions of a detector leaking current while during test beam periods, the gamma background is measured to correlate the evolution of rate capability at different stages of irradiation, hence, monitoring any degradation of the performance due to ageing. In the absence of other signs of ageing, a reduction of the rate capability could be related to an increase of the electrodes resistivity. These background measurement are performed thanks to a trigger provided by a pulse generator at a rate of \SI{300}{Hz}, similarily to what was already done at GIF. Moreover, the data acquisition procedure is the same than in the case of efficiency measurements, as a HV scan composed of several runs taken each at a different HV setting will be taken. The HV scan in test beam period will be taken following the same HV settings than the efficiency measurements in order to correlate both results while during irradiation time, only key high voltage points will be taken: standby voltage and working voltage and this not only for the RPCs operated in double-gap mode but also single-gap, by keeping one of the two RPC layers in standby, in order to spot ageing at the gap level. To rapidly integrate an as great as possible time, all signals contained within a time window of \SI{10}{\mu s} prior to the random trigger signal are extracted form the TDC buffers and saved for further analysis.
	
	In parallel to the DAQ flow is the CAEN flow, monitoring the values of high voltage and currents during the duration of each runs of the HV scans. In the GIF++ RPC data base, the CAEN data is then saved into a ROOT file associated to every DAQ ROOT file. But the CAEN flow doesn't necessary need to be correlated to the DAQ flow. Indeed, during continuous irradiation, the DAQ only is a secondary process. The high voltage and current need to be recorded at all time while the rates only are measured once a day for a short period. Thus, CAEN data is continously saved into the GIF++ database and displayed online thanks to a JS script. The data is then used to calculate and update the integrated charge on a daily basis. When a short HV scan at standby and working voltage is started every night to monitor once a day the gamma background, a CAEN ROOT file is created for each DAQ ROOT file just as explained ealier. Finally, the slow DIP flow is continuously accessed by the CAEN flow in order to correct the high voltage for pressure and temperature during data taking, preventing for environment related performance variations.
	
	\subsection{Data structure}
	\label{chapt5:ssec:datastructure}
