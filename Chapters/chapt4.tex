...
\endgroup
	...
	\begin{figure}[H]
		...
		\caption{\label{fig:GEM-Trigger} Simulated \subref{fig:GEM-Muon:A} efficiency and \subref{fig:GEM-Muon:B} rate of the standalone Level-1 muon trigger using tracks reconstructed in CSCs and all GEM stations compared with Phase-1 values in the case where only CSCs are used or CSCs+GE1/1. The zones of inefficiency of the CSC subsystem are compensated by the addition of GEMs during Phase-2 and the trigger rates is kept from increasing due to the high luminosity~\cite{PHASEIITP}.}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}{0.5\linewidth}
			...
			\caption{\label{fig:GEM-Muon:B}}
		\end{subfigure}
		\caption{\label{fig:GEM-Muon} \subref{fig:GEM-Muon:A}: Simulated resolution of the muon direction measurement $\Delta\phi$ with Phase-2 conditions. In the second endcap station, the resolution is compared in the case of CSCs (ME2/1) alone and CSCs+GEMs (GE2/1+ME2/1) while a similar resolution measurement is given in the case of the first station (GE1/1+ME1/1)~\cite{PHASEIITP}. \subref{fig:GEM-Muon:B}: The addition of GEM detectors on stations 1 and 2 (ME0 is considered to contribute to station 1) as redundant system to CSCs allows improving the muon momentum improvement through a more accurate measurement of the local bending angles $\phi_1$ and $\phi_2$~\cite{PHASEIITP}.}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{\label{fig:Endcap-trigger-eff} Simulation of the impact of RPC hit inclusion onto the local trigger primitive efficiency in station 3~\subref{fig:Endcap-trigger-eff:A} and station 4~\subref{fig:Endcap-trigger-eff:B}~\cite{PHASEIITP}. The contribution of iRPC starts above \psrape{1.8}.}
	\end{figure}
...
\endgroup
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{\label{fig:EMTF-eff} Efficiency of the L1 trigger in the endcap region after Phase-2 upgrade in the case CSC/GEM/RPC hits are requested in at least two stations out of four~\subref{fig:EMTF-eff:A} and in all four stations~\subref{fig:EMTF-eff:B}~\cite{PHASEIITP}.}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{\label{fig:CSC-GEM-perf} Comparison of L1 trigger performances for prompt muons with and without the addition of GEMs in the region \psrapr{2.1}{2.4} at Phase-2 conditions~\cite{PHASEIITP}. GEMs would allow a reduction of the trigger accept rate by an order of magnitude~\subref{fig:CSC-GEM-perf:A} while increasing the trigger efficiency~\subref{fig:CSC-GEM-perf:B}.}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{\label{fig:CSC-GEM-res} The angular resolution on reconstructed muon tracks in the GEM overlap region \psrapr{2.0}{2.15} is compared for Phase-2 conditions in the case CSC are alone~\subref{fig:CSC-GEM-res:A} and in the case the GEMs' data, including ME0, is combined to which of CSCs~\subref{fig:CSC-GEM-res:B}~\cite{PHASEIITP}.}
	\end{figure}
...
\begingroup\setlength{\intextsep}{0pt}\setlength{\columnsep}{15pt}
	...
	\begin{wrapfigure}{O}{.5\linewidth}
		...
		\end{subfigure}
		\caption{\label{fig:UGent-CO2} Efficiency~\subref{fig:UGent-CO2:A}~\cite{PICCOLO2016} and cluster size~\subref{fig:UGent-CO2:B} of a standard double-gap RPC operated with $CO_2$ mixtures for different ratios of $SF_6$.}
	\end{wrapfigure}
...
\begingroup\setlength{\intextsep}{0pt}\setlength{\columnsep}{15pt}
	...
	\begin{wrapfigure}{O}{.5\linewidth}
		\centering
		\includegraphics[width=.9\linewidth]{fig/chapt4/HFO-UGent.png}
...
\begingroup\setlength{\intextsep}{5pt}\setlength{\columnsep}{15pt}
	...
	\begin{wrapfigure}{O}{.5\linewidth}
		\centering
		\includegraphics[width=\linewidth]{fig/chapt4/iRPC-HFO-mixture.pdf}
