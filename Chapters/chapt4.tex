...
\label{chapt4:sec:principle}
	...
	{\color{blue} [Talk about the active volume inside of the gas gap : due to the exponential growth of the avalanch, only the first few percent of the gas volume will be able to give enough gain to see a strong enough signal.]}
...
\label{sec:RateCapa}
	...
	RPCs where developed early 1980s. At that time it was using an operating mode now referred to as \textit{streamer mode}. Streamers are large discharges that develop in between the 2 electrodes enough to connect them and discharge them locally. Indeed, electrodes, though they are a unique volume of resistive material, can be assimilated to capacitors. At the moment an electric field is applied in between their outer surfaces, the charge carriers inside of the volume will start moving leading eventually to a situation where there is no voltage across the electrodes. Finally, when a streamer discharge occurs, the negative charge of the electrons on the anode and the positive charges of the cations on the cathode will locally affect and reduce the electric field, quenching the streamer and preventing a spark to connect the two electrodes and fully discharge them by release the full charge contained in both of them. This can be understood through Figure~\ref{fig:ElecCharge}~\cite{CROTTY93}.
