...
each digit being 1 or 0. Bellow is given an example with a 4 TDCs setup.
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{\label{fig:qflag-daq} The effect of the quality flag is explained by presenting the content of \cppinline{TBranch number_of_hits} of a data file without \cppinline{Quality_flag} in Figure~\ref{fig:qflag-daq:A} and the content of the same \cppinline{TBranch} for data corresponding to a \cppinline{Quality_flag} where all TDCs were labelled as \cppinline{GOOD} in Figure~\ref{fig:qflag-daq:B} taken with similar conditions. It can be noted that the number of entries in Figure~\ref{fig:qflag-daq:B} is slightly lower than in Figure~\ref{fig:qflag-daq:A} due to the excluded events.}
	\end{figure}
