...
\vspace{5mm}
	...
	\dirtree{%
	 ...
	 .2 build\DTcomment{build folder}.
	 .3 ....
	 ...
	 .2 CMakeLists.txt\DTcomment{compiler instructions}.
	 .2 config.h.in\DTcomment{code version}.
	 .2 Doc-DAQ-v5.pdf\DTcomment{documentation (this appendix)}.
...
\label{app1:sec:setup}
    ...
    \hspace{-15mm}
		...
		\end{subfigure}
		\caption{\label{fig:DAQSetup} \subref{fig:DAQSetup:A} View of the front panel of a V1190A TDC module~\cite{V1190AMUT}. \subref{fig:DAQSetup:B} View of the front panel of a V1718 Bridge module~\cite{V1718MUT}. \subref{fig:DAQSetup:C} View of the front panel of a 6U 6021 VME crate~\cite{6U6000MUT}.}
	\end{figure}
...
\label{app1:sec:Data}
	...
	The v1190a object, defined in the DAQ software as in Source Code~\ref{cpp:v1190a}, offers the possility to store all TDCs in the readout setup into a single object containing a list of hardware addresses (addresses to access the TDCs' buffer through the VME crate) and each constructor and method acts on the list of TDCs to set the different acquisition parameters as describe above. The type of trigger matching is chosen with \cppinline{v1190a::SetTrigMatching()} and the time substraction, used to have a time measurement referring to the beggining of the time window, is set by \cppinline{v1190a::SetTrigTimeSubstraction()}. Then, the wiwdow width and offset are respectively set thanks to \cppinline{v1190a::SetTrigWindowWidth()} and \cppinline{v1190a::SetTrigWindowOffset()}. The rejection and extra search margin, even if left unused and hence set to a default value of 0, can be set through \cppinline{v1190a::SetTrigRejectionMargin()} and \cppinline{v1190a::SetTrigSearchMargin()}. These methods are then called in \cppinline{v1190a::SetTrigConfiguration()} that uses the information contained in the configuration file \cppinline{IniFile *inifile} to set the different TDC parameters. A thorough explaination of the content of the configuration file is provided in Section~\ref{app1:ssec:inifile}.\\
	
	Among the other methods of class \cppinline{v1190a} can be found a set of the detection mode (\cppinline{v1190a::SetTDC DetectionMode()}), of the TDC time resolution (\cppinline{v1190a::SetTDCResolution()}), of the dead time in between two consecutive signals recorded into a single channel (\cppinline{v1190a::SetTDCDeadTime()}) or of the maximal number of signals that can be recorded per event (\cppinline{v1190a::SetTDCEventSize()}). To help with setting these parameters, \cppinline{enum} were used (\cppinline{EdgeMode}, \cppinline{Resolution}, \cppinline{DeadTime} and \cppinline{HitMax} are defined in \textinline{include/v1190a.h}).\\
...
class v1190a
{
 ...
 public:
	...
	\begin{itemize}
		...
		\item \textbf{a TDC trailer} which is enabled thanks to \cppinline{v1190a::SetTDCHeadTrailer()},
...
for(Uint i=0; i<TDCData.EventList->size(); i++){
	...
	\begin{itemize}
		\item[] TDC 0: \cppinline{QFlag} $= 10^0 \times$ \cppinline{QualityFlag}
		\item[] TDC 1: \cppinline{QFlag} $= 10^1 \times$ \cppinline{QualityFlag}
		\item[] ...
		\item[] TDC N: \cppinline{QFlag} $= 10^N \times$ \cppinline{QualityFlag}
	\end{itemize}
...
each digit being 1 or 0. Bellow is given an example with a 4 TDCs setup.
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{\label{fig:qflag-daq} The effect of the quality flag is explained by presenting the content of \cppinline{TBranch number_of_hits} of a data file without \cppinline{Quality_flag}~\subref{fig:qflag-daq:A} and the content of the same \cppinline{TBranch} for data corresponding to a \cppinline{Quality_flag} where all TDCs were labelled as \cppinline{GOOD}~\subref{fig:qflag-daq:B} taken with similar conditions. It can be noted that the number of entries in Figure~\subref{fig:qflag-daq:B} is slightly lower than in Figure~\subref{fig:qflag-daq:A} due to the excluded events.}
	\end{figure}
	...
	\begin{figure}[H]
		...
		\end{subfigure}
		\caption{\label{fig:qflag-kodel} Using the same data as previously showed in Figure~\ref{fig:qflag-daq}, the effect of the quality flag is explained by presenting the reconstructed hit multiplicity of a data file without \cppinline{Quality_flag}~\subref{fig:qflag-kodel:A} and the reconstructed content of the same RPC partition for data corresponding to a \cppinline{Quality_flag} where all TDCs were labelled as \cppinline{GOOD}~\subref{fig:qflag-kodel:B} taken with similar conditions. The artificial high content of bin 0 is completely suppressed.}
	\end{figure}
...
\label{app1:sec:Comm}
    ...
    In the previous section, the data transfer as been discussed. The importance of the \cppinline{v1718} object (Source Code~\ref{cpp:v1718}), used as private member of \cppinline{DataReader}, was not explicited. VME  master modules are used for communication purposes as they host the USB port that connects the powered crate buffer to the computer were the DAQ is installed. From the source code point of view, this object is used to control the communication status, by reading the returned error codes with \cppinline{v1718::CheckStatus()}, or to check for IRQs coming from the TDCs through \cppinline{v1718::CheckIRQ()}. To ensure that triggers are blocked at the hardware level, a NIM pulse is sent out of one the two first programmable outputs of the module (\cppinline{v1718::SendBUSY()}) to the VETO of the coïndidence module where the trigger signals originate. As long as this signal is ON, no trigger can reach the TDCs anymore. Finally, used in the case of noise and background measurements in which the trigger needs not to be provided by the muon bean but by an uncorrelated source, a pulse generator is enabled with \cppinline{v1718::RDMTriggerPulse()}. The "random" pulse is sent through the third and fourth outputs of the module. Both the BUSY signal and the random pulse are shaped in the method \cppinline{v1718::SetPulsers()} where the number of pulses to be generated, their width, as well as the period of the pulse generator is defined.\\
...
class IniFile{
 ...
 FileData[token] = value;
    ...
    \begin{itemize}
    	...
    	\item[•] \textinline{KILL}, command to kill data taking sent by user and read via function \cppinline{CheckKILL()}. Note that the DAQ doesn't stop before the current ROOT file is safely written and saved.
    \end{itemize}
